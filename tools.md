# The Tools Used in `angularjs-admin-base`

## Introduction

`angularjs-admin-base` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `angularjs-admin-base` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`angularjs-admin-base` uses git to manage its codebase. While in theory you don't have
to use Git once you download `angularjs-admin-base`, this project makes the assumption
...
While `angularjs-admin-base` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `angularjs-admin-base` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `angularjs-admin-base`; Grunt is
...
to get started with `angularjs-admin-base`. But for those curious or looking to go a
...
Grunt is the engine behind `angularjs-admin-base`. It's the magic that makes it move.
...
`angularjs-admin-base` comes with a `bower.json` file that looks something like this:

```js
{
  "name": "angularjs-admin-base",
...
in `angularjs-admin-base` by default in there to see what I mean.
